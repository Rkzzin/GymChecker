...
export function CreateMemberModal({ isOpen, onClose, onSuccess, plans, darkMode }: CreateMemberModalProps) {
	...
	// Seleciona o primeiro plano por padrão quando a lista de planos muda ou o modal abre
	useEffect(() => {
		...
		<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-opacity">
			<div className={`rounded-xl shadow-2xl w-full max-w-md p-6 ${cardClass} border animate-in fade-in zoom-in duration-200`}>
				...
				<form onSubmit={handleCreateMember} className="space-y-4">
					...
					<div className="flex gap-3 pt-2"><button type="button" onClick={onClose} className={`flex-1 py-2.5 rounded-lg font-medium transition-colors ${darkMode ? 'bg-gray-800 hover:bg-gray-700' : 'bg-gray-100 hover:bg-gray-200'}`}>Cancelar</button><button type="submit" disabled={submitting} className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-2.5 rounded-lg font-bold shadow-lg shadow-orange-500/20">{submitting ? '...' : 'Confirmar'}</button></div>
				</form>
			</div>
		</div>
	);
