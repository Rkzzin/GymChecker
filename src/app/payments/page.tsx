...
export default function Payments() {
	...
	const fetchPayments = async () => {
		setLoading(true);
		try {
			const startDate = new Date(yearFilter, monthFilter - 1, 1).toISOString();
			const endDate = new Date(yearFilter, monthFilter, 0, 23, 59, 59).toISOString();
		...
		try {
			const { error } = await supabase
				...
				.update({
					...
					payment_date: new Date(paymentToEdit.payment_date).toISOString(),
	...
	// Classes Globais (iguais ao Dashboard)
			...
			<main className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
				...
				<div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
					<div>
						...
						<p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
							Gerencie e audite todas as entradas financeiras.
						</p>
					</div>
					...
					<div className={`flex items-center gap-4 px-4 py-3 rounded-lg border ${cardClass}`}>
						...
						<div className="text-xl font-bold text-green-500">{formatCurrency(totalMonth)}</div>
					</div>
				</div>
				...
				<div className="flex justify-end gap-3 mb-6">
					<select
						...
						className={`h-10 pl-3 pr-8 rounded-md border text-sm focus:ring-2 focus:ring-orange-500 outline-none transition-all ${inputClass}`}
					>
					...
					<table className="w-full text-sm">
						...
						<tbody className="divide-y divide-gray-200 dark:divide-gray-800">
							{loading ? (
								...
								<tr><td colSpan={6} className="p-8 text-center text-gray-500">Nenhum pagamento encontrado neste período.</td></tr>
							) : (
								payments.map(pay => (
									<tr key={pay.id} className={`group ${darkMode ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'} transition-colors`}>
										<td className="px-6 py-4 whitespace-nowrap opacity-80">
											{new Date(pay.payment_date).toLocaleDateString('pt-BR', { timeZone: 'UTC' })}
										</td>
										...
										<td className="px-6 py-4">
											<span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium uppercase tracking-wide ${getMethodBadgeClass(pay.method || '')}`}>
												{pay.method}
											</span>
										</td>
										<td className="px-6 py-4 text-xs italic opacity-60 max-w-xs truncate">
											{pay.notes || '-'}
										</td>
										<td className="px-6 py-4 text-center">
											<button
												onClick={() => openEditModal(pay)}
												className="text-gray-400 hover:text-orange-500 transition-colors p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800"
				...
				<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 transition-all">
					<div className={`rounded-xl shadow-2xl w-full max-w-md p-6 ${cardClass} border ${darkMode ? 'border-gray-700' : 'border-gray-200'}`}>
						<div className="mb-6">
							...
							<p className={`text-sm mt-1 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
								Aluno: <span className="font-medium text-orange-500">{paymentToEdit.customerName}</span>
							</p>
						</div>
						...
						<form onSubmit={handleUpdatePayment} className="space-y-5">
							...
							<div>
								...
								<select
									...
									onChange={e => setPaymentToEdit({ ...paymentToEdit, method: e.target.value })}
									className={`w-full border p-2.5 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all ${inputClass}`}
								>
							...
							<div className="flex gap-3 pt-2">
								...
								<button
									type="submit"
									disabled={submitting}
									className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-2.5 rounded-lg font-bold shadow-lg shadow-orange-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
								>
